...
import { Transaction, PublicKey } from "https://esm.sh/@solana/web3.js@1.98.4";
import { 
  getConnection, 
  decodePrivateKey,
  buildInstruction,
...
serve(async (req) => {
  ...
  try {
    const { taskPubkey, ownerTokenAccount } = await req.json();
    ...
    console.error('Error in solana-accept-task:', error);
    return new Response(
      JSON.stringify({ error: 'Failed to accept task', details: error instanceof Error ? error.message : 'Unknown error' }),
